    ...
    async def start(self):
        ...
        if FORCE_SUB_GROUP:
            try:
                ...
                self.LOGGER(__name__).warning(a)
                self.LOGGER(__name__).warning(
                    "Bot failed to Fetch invite link from FORCE_SUB_GROUP!"
                )
                self.LOGGER(__name__).warning(
        ...
        try:
            ...
            self.LOGGER(__name__).warning(
                f"Make sure @{self.username} is admin in your DataBase Channel, Current CHANNEL_ID: {CHANNEL_ID}"
            )
            self.LOGGER(__name__).info(
                "Bot Stopped. Join Group https://t.me/SharingUserbot for Assistance"
            )
        ...
        self.LOGGER(__name__).info(
            f"[🔥 ACTIVATED SUCCESSFULLY! 🔥]\n\nBOT Created by @{OWNER}\nIf @{OWNER} Needs Assistance, Feel Free to Ask in Group https://t.me/SharingUserbot"
        )
